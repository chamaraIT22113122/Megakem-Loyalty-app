...
function App() {
  ...
  const [rawScanHistory, setRawScanHistory] = useState([]);
  ...
  const scanHistory = useMemo(() => {
    return rawScanHistory.map(scan => {
  ...
  const recalculateScanPrices = (scans, currentProducts) => {
    return scans.map(scan => {
      ...
      // Update price if product found (always use real-time catalog price for dashboard)
      if (product && product.price > 0) {
        return { ...scan, price: product.price };
      }
  ...
  const refreshScanHistory = async () => {
    ...
    try { 
      ...
      // Remove recalculateScanPrices usage because it is now handled by useMemo dynamically
      setRawScanHistory(scans);
  ...
  const loadAdminData = async () => {
    ...
    try {
      ...
      setRawScanHistory(scansRes.data?.data || []);
  ...
  const handleDeleteScan = async () => {
    ...
    try {
      ...
      setRawScanHistory(rawScanHistory.filter(s => s._id !== deleteDialog.scanId));
  ...
  return (
    <ThemeProvider theme={getTheme()}><CssBaseline /><Box sx={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f5f7fa 0%, #e8f0f7 100%)', display: 'flex', flexDirection: 'column' }}>
      ...
      <Container maxWidth={view === 'admin' && adminAuth ? 'xl' : 'xl'} sx={{ flexGrow: 1, py: { xs: 2, sm: 3 }, px: { xs: 2, sm: 3 }, display: 'flex', flexDirection: 'column' }}>
        ...
        {view === 'scanner' && <Box sx={{ maxWidth: '600px', width: '100%', mx: 'auto' }}><Paper sx={{ flexGrow: 1, bgcolor: '#000', color: 'white', overflow: 'hidden', position: 'relative', borderRadius: { xs: 2, sm: 3 }, display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        ...
        {view === 'admin' && !adminAuth && <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '100%', px: 2, animation: 'fadeIn 0.5s ease-in', '@keyframes fadeIn': { from: { opacity: 0, transform: 'translateY(20px)' }, to: { opacity: 1, transform: 'translateY(0)' } } }}>
          <Box sx={{ maxWidth: '440px', width: '100%', mx: 'auto' }}>
